...
function Post({data}) {
    ...
    let comentarioFiltrado = Object.values(comentarios).filter(c => c.QuestionId == data.id);
        ...
        <CardPost >
            ...
            <footer>
                <h3 onClick={toggleComents}>
                    {comentarioFiltrado.length === 0 ? "Seja o primeiro a comentar":
                    `${comentarioFiltrado.length} Comentário${comentarioFiltrado.length > 1 ? "s" : `` }`
                    }
                </h3>
                {showComents && (
                    <>
